...
pow_lux <- water %>%
  select(flux, power, temp) %>%
  filter(flux < 700 & flux > 0 & power > 0)
...
ggplot(pow_lux,) + geom_point(aes(flux, power, col=temp)) + scale_color_viridis() + labs(x = "Flux (W/m^2)", y = "Power (W)", title = "Solar Power Generation") +  theme(plot.title = element_text(hjust = 0.5)) + geom_abline(slope = lem$coefficients[2], intercept = lem$coefficients[1]) + geom_text(aes(3, 40, label = paste("Adj R2 = ", adj.r.squared, "\n", "Intercept =",intercept, "\n", "Slope =", slope, "\n", "P =", pvalue)))
