...
```
```{r}
# Calibration of Current (not actually used)
df_cal <- consolidated_data %>%
  dplyr::select(current_raw, temp, voltage)%>%
  filter(current_raw > -1, voltage < 0.5, current_raw < 0.7)%>%
  mutate(dt = 27 - temp + 0.2868533)
  
cal_fit = lm(current_raw ~ dt, df_cal)
plot(cal_fit)
plot(df_cal$current_raw, df_cal$dt)
```
